function [P] = social_interaction_model_base(G,Comms,meta_data)
...
%
% Since we have Lclass and L as defined we might as well deal with the
% model parameter pclssyr as defined above...that parameter is for
% classyear interactions between individuals who are NOT friends
for ii = 1:4
    % calculate the proper value for the classyear
    classsize = sum(Lclass(ii,:));
    fact_ii = (pclssyr*classsize)/(7*13);
    % get a logical matrix representing "same classyear but not friends"
    L2(Lclass(ii,:),Lclass(ii,:)) = true;
    tempLogical = ~L&L2;
    % set the denominator as classsize-1....I know we could just use
    % classsize and then let that value cancel....but I'm actually doing it
    % this way to show the intuition behind the estimation of this
    % particular probability value....just the per diem / total people that
    % the individual could contact in his/her class
    P(tempLogical) = P(tempLogical) + fact_ii/(classsize-1);
    L2(Lclass(ii,:),Lclass(ii,:)) = false;
end
% we can clear tempLogical Lclass and L2, we don't need those anymore
clear tempLogical Lclass and L2
%--------------------------------------------------------------------------
% OK, now we need to deal with the parameter phsng (see definition above)
% in a similar way to what we just did for pclassyr
% 
% First we'll nee the unique values of the anonymized housing identifiers
% from the metadata variable meta_data
IDs = unique(meta_data(:,5));
% the zero value indicates that information was unknown for some
% individuals
if IDs(1)==0
    IDs(1) = [];
end
% use logical arrays similar to what we did above for the parameter pclassyr
L2 = false(n,n);
for ii = 1:length(IDs)
    houseLogical = (meta_data(:,5)==IDs(ii))';
    housesize = sum(houseLogical);
    L2(houseLogical,houseLogical) = true;
    tempLogical = ~L&L2;
    P(tempLogical) = P(tempLogical) + phsng/(housesize-1); %<-- using same intuition as above
    L2(houseLogical,houseLogical) = false;
end
% Now we can assign the random interaction factor for non-friends in
% general
P(~L) = P(~L) + prnd;
