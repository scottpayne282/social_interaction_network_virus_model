function [fig,bttns,hslide,cntctObs] = visualize_simulation(day,G,Comms,Y,meta_data)
...
% allows the user to scroll easily back and forth and see how the virus
% spread relates to the social structures of the network of daily in-person
% contacts.
%
% INPUTS:
%       day is the Matlab structure type variable that stored the
%       information about the simulation. So day(i).contacts is a Matlab
%       graph object recording the in-person contacts on day i. And
%       day(i).status is a 3826x1 numeric that records the status for each
%       individual on day i.
%       status codes:
%       0 -- susceptible
%       1 -- infected
...
fig.UserData.statusObs = gobjects(1,4);
...
addlistener(hslide,'ContinuousValueChange',@(hObject, event) ...
    doit(hObject,fig,numday,f0,fout,cntctObs,day,Y,yellow,orange,pink,green));           
...
%--------------------------------------------------------------------------
function doit(hObject,fig,maxval,f0,fout,cntctObs,day,Y,yellow,orange,pink,green)
% value of "t"
t = get(hObject,'Value');
...
if (t-fig.UserData.crrday) ~= 0
    ...
    if t~=0
        ...
        if prev>0
            delete(fig.UserData.statusObs(1));
            delete(fig.UserData.statusObs(2));
            delete(fig.UserData.statusObs(3));
            delete(fig.UserData.statusObs(4));
        ...
        sts4 = status==4';
        if any(sts1)
            fig.UserData.statusObs(1) = plot(Y(sts1,1),Y(sts1,2),'o','MarkerEdgeColor',yellow,'MarkerFaceColor',yellow,'MarkerSize',2.8);
        end
        if any(sts2)
            fig.UserData.statusObs(2) = plot(Y(sts2,1),Y(sts2,2),'o','MarkerEdgeColor',orange,'MarkerFaceColor',orange,'MarkerSize',2.3);
        end
        if any(sts3)
            fig.UserData.statusObs(3) = plot(Y(sts3,1),Y(sts3,2),'d','MarkerEdgeColor',pink,'MarkerFaceColor',pink,'MarkerSize',3.3);
        end
        if any(sts4)
            fig.UserData.statusObs(4) = plot(Y(sts4,1),Y(sts4,2),'o','MarkerEdgeColor',green,'MarkerFaceColor',green,'MarkerSize',2.3);
        end
        hold off
